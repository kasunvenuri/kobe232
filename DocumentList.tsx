      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {documents.map((doc, idx) => {
          ...
          return (
            <div
              key={doc.id}
              className="bg-slate-50 border border-slate-200 rounded-lg p-3.5 flex flex-col justify-between hover:border-yellow-400 transition-all shadow-2xs group"
            >
              {/* Top Header info */}
              <div>
                ...
                <div className="mb-3">
                  {doc.status === 'processing' && (
                    <div className="flex items-center space-x-1.5 text-xs text-yellow-700 font-semibold bg-yellow-50 px-2.5 py-1 rounded border border-yellow-100">
                    ...
                    {doc.extractedItems.length === 0 ? (
                      ...
                      doc.extractedItems.slice(0, 3).map((item) => (
                        ...
                        >
                          ...
                          <span className="font-mono text-yellow-600 font-bold text-[11px] shrink-0 bg-yellow-50 px-1.5 py-0.2 rounded border border-yellow-100">
                            {item.qty}
                          </span>
                        </div>
                      ))
              ...
              <div className="pt-2 border-t border-slate-200/80 flex items-center justify-between gap-1 text-xs">
                ...
                {doc.status === 'success' && (
                  <button
                    ...
                    className="px-2.5 py-1 rounded bg-yellow-50 hover:bg-yellow-100 border border-yellow-200 text-yellow-700 font-bold uppercase text-[10px] flex items-center gap-1 transition-colors cursor-pointer"
                  >
                    <Edit3 className="w-3 h-3 text-yellow-600" /> Edit Items
                  </button>
                )}
