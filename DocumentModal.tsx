      ...
      <div className="bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-slate-950">
          ...
          <div className="flex-1 relative overflow-auto p-6 flex items-center justify-center bg-slate-950/80">
            <div
              style={{ transform: document.type === 'application/pdf' ? 'none' : `scale(${zoom})`, transformOrigin: 'center center' }}
              className="transition-transform duration-150 ease-out max-w-full w-full h-full flex items-center justify-center"
            >
              {document.type === 'application/pdf' ? (
                <object
                ...
                >
                  <p className="text-white text-sm text-center">Unable to display PDF. Please check your browser settings.</p>
                </object>
              ) : (
                <img
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1">
              {document.extractedItems.length === 0 ? (
                ...
                document.extractedItems.map((item, idx) => (
                  <div
                    key={item.id}
                    className="bg-slate-800/80 border border-slate-700/60 rounded p-3 text-xs space-y-1 hover:border-yellow-500/50 transition-colors"
                  >
